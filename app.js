...
function Birth(priority){
 var pri = priority;
 $("#formTable").empty();
 $("#formTable").append("<div id='step1'><p>準備中・・・</p><p>何かのタマゴが産み落とされたようです</p></div><a href='' onclick='newmon("+pri+");'><img src='../img/tamago.png' width='200px' height='auto' alt='たまご'></a>")
}
...
function newmon(pri){
 ...
 saveData.order("createDate",true)
         ...
         //テーブルにデータをセット
          ...
          $("#formTable").empty();
          $("#formTable").append("<div id='step2'><p id='tap'>新しいタスモンが誕生しました！</p><p>"+monname+"</p></div><img src='../img/nor_mon/tasmon"+pri+"-1.svg' alt='"+monname+"'><div class='box'><p>タスク名: "+name+"</p></div><div class='box'><p>期限: "+lim+"</p></div><input type='button' value='OK' onclick='window.location.reload();'>");
         })
}
